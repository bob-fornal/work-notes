...
* It is a managed database service for databases that use SQL as a query langage.

It allows creation of databases in the cloud that are managed by AWS ...
...
* A Read Replica can be created to run the new workload from.
...
* Failover in case of loss of AZ, loss of netowkr, instance, or storage failure.
...
* Zero downtime operatrion (no need to stop the database).
...
* Possility to encrypt the master and read replicas with AWS-KMS - AES-256 encryption.
* Encryption has to be defined at launch time.
* **If the master is not encrypted, the read replicas cannot be encrypted**.
* Transparend Data Encryption (TDE) available for Oracle and SQL Server.
...
* IAM to centrally manage users instead of database.
...
* Failover in Aurora is instantaneous. It is High Availability native.
...
* **Possibliity to authenticate using IAM token (same method as RDS)**.
...
* In case immediate failover for write node (High Availability).
...
* Application queries ElastiCache. If the data is not available, get from RDA and stores in ElastiCache.
